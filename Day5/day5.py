...
def mapp(input: int, mapping: List[int]) -> int:
    for elem in mapping:
        if elem[1] <= input and elem[1] + elem[2] > input:
            return elem[0] - elem[1] + input
    return input

def lowest(almanac: Dict[str, List[List[int]]]) -> int:
    locations = []
    ...
    for seed in almanac["seeds"]:
        output = mapp(seed, seed_to_soil)
        output = mapp(output, soil_to_fertilizer)
        output = mapp(output, fertilizer_to_water)
        output = mapp(output, water_to_light)
        output = mapp(output, light_to_temperature)
        output = mapp(output, temperature_to_humidity)
        output = mapp(output, humidity_to_location)
